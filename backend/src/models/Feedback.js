...
class Feedback {
  ...
  // Create new feedback
  static async create({ userId, module, submodule, title, description, type, priority = 'medium' }) {
    const query = `
      INSERT INTO feedback (user_id, module, submodule, title, description, type, priority)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `;
    const result = await pool.query(query, [userId, module, submodule, title, description, type, priority]);
    return result.rows[0];
  }
