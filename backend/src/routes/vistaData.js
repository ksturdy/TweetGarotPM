...
// ==================== AUTOMATED UPLOAD (API KEY AUTH) ====================

// API Key authentication for automated scripts
const apiKeyAuth = (req, res, next) => {
  ...
  // Set a system user context for automated uploads
...
// POST /api/vista/import/upload-auto - Automated upload endpoint (API key auth)
router.post('/import/upload-auto', apiKeyAuth, handleUpload, async (req, res, next) => {
  let tempFilePath = null;
  try {
    if (!req.file) {
      return res.status(400).json({ message: 'No file uploaded' });
    ...
    // Process each known sheet type
    const sheetConfigs = [
      { name: 'TGPBI_PMContractStatus', type: 'contracts' },
      { name: 'TGPBI_SMWorkOrderStatus', type: 'work_orders' },
      { name: 'TGPREmployees', type: 'employees' },
      { name: 'TGARCustomers', type: 'customers' },
      { name: 'TGAPVendors', type: 'vendors' }
    ];

    for (const config of sheetConfigs) {
      if (sheetNames.includes(config.name)) {
        console.log(`[Vista Auto-Import] Processing ${config.name}...`);
        const data = loadSheet(config.name);
        results.sheetsProcessed.push(config.name);
        results.records[config.type] = { rows: data.length };
        console.log(`[Vista Auto-Import] ${config.name}: ${data.length} rows`);
      }
    }
    ...
    res.json({
      success: true,
      message: `Processed ${results.sheetsProcessed.length} sheets`,
