...
function generatePhaseReportPdfHtml(rows, filters = {}) {
  ...
  const tableRows = rows.map((r, i) => {
    ...
    const bg = r.is_missing ? '#fff7ed' : (i % 2 === 0 ? '#ffffff' : '#f8fafc');
    const jobColor = r.is_missing ? '#92400e' : '#1e3a5f';
    return `
      <tr style="background:${bg};">
        <td style="padding:5px 6px; font-weight:600; color:${jobColor}; white-space:nowrap;">${r.job_number || '-'}</td>
        <td style="padding:5px 6px; color:${r.is_missing ? '#92400e' : '#334155'};">${r.job_name || '-'}</td>
        ...
        <td style="padding:5px 6px; color:${r.is_missing ? '#b45309' : '#475569'}; font-style:${r.is_missing ? 'italic' : 'normal'};">${r.phase_name || '-'}</td>
        ...
        <td style="padding:5px 6px; text-align:right; font-weight:600;">${r.is_missing ? '-' : fmtHours(r.est_hours)}</td>
        <td style="padding:5px 6px; text-align:right; font-weight:600;">${r.is_missing ? '-' : fmtHours(r.jtd_hours)}</td>
        <td style="padding:5px 6px; text-align:right; font-weight:600; color:${varColor};">${r.is_missing ? '-' : (variance !== null ? variance.toFixed(1) + '%' : '-')}</td>
      </tr>`;
  }).join('');
...
<body>
  ...
  <div class="header">
    ...
    <div class="header-date">Generated: ${dateLabel}<br>${distinctJobs} job${distinctJobs !== 1 ? 's' : ''} · ${phaseRowCount} phase${phaseRowCount !== 1 ? 's' : ''}</div>
  </div>
  ...
  <table>
    ...
    <tfoot>
      <tr>
        <td colspan="8" style="font-weight:700;">TOTAL (${distinctJobs} jobs · ${phaseRowCount} phases)</td>
