...
const FieldPipingFittingOrderDetail: React.FC = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => pipingFittingOrdersApi.delete(Number(id)),
    onSuccess: () => {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        <div>
          <h1 className="field-page-title">FO-PIP-{order.number}</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8, marginBottom: 24 }}>
        ...
        {isDraft && (
          <button
            className="field-btn field-btn-secondary"
            onClick={() =>
              navigate(`/field/projects/${projectId}/piping-fitting-orders/${id}/edit`)
            }
          >
...
export default FieldPipingFittingOrderDetail;
