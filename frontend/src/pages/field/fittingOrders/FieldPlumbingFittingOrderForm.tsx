...
const FieldPlumbingFittingOrderForm: React.FC = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Partial<PlumbingFittingOrder>) =>
      plumbingFittingOrdersApi.update(Number(id), data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['field-plumbing-fitting-orders'] });
      queryClient.invalidateQueries({ queryKey: ['field-plumbing-fitting-order', id] });
      navigate(`/field/projects/${projectId}/plumbing-fitting-orders/${id}`);
    },
  });
  ...
  return (
    <div>
      ...
      <p className="field-page-subtitle">Plumbing fabrication request</p>
      ...
      <div className="field-form-section">
        ...
        <div className="field-form-group">
          ...
          <input
            ...
            onChange={handleChange}
            placeholder="1st Floor Restroom"
          />
        </div>
      </div>
      ...
      <div className="field-form-section">
        ...
        <div className="field-form-row">
          <div className="field-form-group">
            <label className="field-form-label">Fixture Type</label>
            <input
              className="field-form-input"
              name="fixture_type"
              value={form.fixture_type || ''}
              onChange={handleChange}
              placeholder="Water Closet"
            />
          </div>
          <div className="field-form-group">
            <label className="field-form-label">Rough-In Dimensions</label>
            <input
              className="field-form-input"
              name="rough_in_dimensions"
              value={form.rough_in_dimensions || ''}
              onChange={handleChange}
              placeholder='12" center'
            />
          </div>
        </div>
      </div>
