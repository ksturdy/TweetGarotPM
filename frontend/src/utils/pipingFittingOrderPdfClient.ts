...
interface PipingFittingOrderData {
  number: number;
  title?: string;
  project_name?: string;
...
export async function generatePipingFittingOrderPdf(order: PipingFittingOrderData): Promise<Blob> {
  const doc = new jsPDF({
  ...
  doc.setFontSize(16);
  doc.setTextColor(0);
  doc.text('PIPING FITTING ORDER', left, y + 16);
  ...
  doc.setFontSize(14);
  doc.setTextColor(0);
  doc.text(`FO-PIP-${order.number || ''}`, right, y + 16, { align: 'right' });
  ...
  doc.text(`Tweet Garot Mechanical | Piping Fitting Order FO-PIP-${order.number || ''}`, left + contentW / 2, y + 6, { align: 'center' });
