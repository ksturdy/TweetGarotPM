...
import { PIPING_ITEMS_CATALOG } from '../../data/pipingItemsCatalog';
...
const shapeStyle = (item: PlaceableItemDef): React.CSSProperties => ({
  ...
  borderRadius: item.shape === 'circle' ? '50%' : item.shape === 'diamond' ? 2 : 3,
  transform: item.shape === 'diamond' ? 'rotate(45deg)' : undefined,
});

export default function PipingPalette() {
  const open = useUiStore((s) => s.showPipingPalette);
  const onClose = useUiStore((s) => s.togglePipingPalette);
  ...
  const filtered = useMemo(() => {
    if (!search.trim()) return PIPING_ITEMS_CATALOG;
    const q = search.toLowerCase();
    return PIPING_ITEMS_CATALOG.filter(
      (item) =>
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '8px 10px' }}>
        <input
          ...
          placeholder="Search piping items..."
          style={{
            width: '100%',
            borderRadius: 6,
                        ...
                        return (
                          ...
                          >
                            <div style={shapeStyle(item)}>
                              <span style={{ transform: item.shape === 'diamond' ? 'rotate(-45deg)' : undefined, fontSize: 7 }}>
