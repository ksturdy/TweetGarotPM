...
interface SellSheetPreviewModalProps {
  sellSheet: SellSheet & { images?: SellSheetImage[] };
  isOpen: boolean;
  onClose: () => void;
}

const SellSheetPreviewModal: React.FC<SellSheetPreviewModalProps> = ({
  sellSheet,
  isOpen,
  onClose,
}) => {
  ...
  const handleDownloadPdf = async () => {
    try {
      const response = await sellSheetsApi.downloadPdf(sellSheet.id);
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `Sell-Sheet-${(sellSheet.service_name || sellSheet.title || '').replace(/[^a-zA-Z0-9]/g, '-')}.pdf`;
...
export default SellSheetPreviewModal;
